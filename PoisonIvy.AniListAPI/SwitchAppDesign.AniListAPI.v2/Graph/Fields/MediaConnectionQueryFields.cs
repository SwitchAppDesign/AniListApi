	...
	public class MediaConnectionQueryFields
	{
        ...
        public GraphQueryField EdgesQueryField(IList<GraphQueryField> fields)
		{
		    ...
		    if (fields.Any(x => x.ParentClassType != typeof(MediaEdgeQueryFields)))
		        throw new GraphQueryFieldInvalidException($"The following fields are not valid query fields for the field ({Edges.GetType().Name}): {fields.Where(x => x.ParentClassType != typeof(MediaEdgeQueryFields)).Select(x => x.GetType().Name).Aggregate((x, y) => $"{x}, {y}")}.");
        ...
        public GraphQueryField NodesQueryField(IList<GraphQueryField> fields)
		{
		    ...
		    if (fields.Any(x => x.ParentClassType != typeof(MediaQueryFields)))
		        throw new GraphQueryFieldInvalidException($"The following fields are not valid query fields for the field ({Nodes.GetType().Name}): {fields.Where(x => x.ParentClassType != typeof(MediaQueryFields)).Select(x => x.GetType().Name).Aggregate((x, y) => $"{x}, {y}")}.");
