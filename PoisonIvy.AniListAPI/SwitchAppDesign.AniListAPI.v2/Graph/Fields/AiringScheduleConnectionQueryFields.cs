	...
	public class AiringScheduleConnectionQueryFields
	{
        ...
        public GraphQueryField EdgesQueryField(IList<GraphQueryField> fields)
	    {
	        if (fields == null || !fields.Any())
	            throw new GraphQueryFieldInvalidException($"Query field ({Edges.GetType().Name}) requires at least one airing schedule edge query field.");

	        if (fields.Any(x => x.ParentClassType != typeof(AiringScheduleEdgeQueryFields)))
	            throw new GraphQueryFieldInvalidException($"The following fields are not valid airing schedule edge query fields {fields.Where(x => x.ParentClassType != typeof(AiringScheduleEdgeQueryFields)).Select(x => x.GetType().Name).Aggregate((x, y) => $"{x}, {y}")}.");
	    ...
	    {
	        if (fields == null || !fields.Any())
	            throw new GraphQueryFieldInvalidException($"Query field ({Nodes.GetType().Name}) requires at least one airing schedule query field.");
